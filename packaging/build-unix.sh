...
# Where README.md tells a Linux user to install the agent binary. The systemd
...
if [ "$platform" = linux ]; then
    ...
    cp packaging/linux/factorseal-agent-start "$stage/$archive/bin/"
    ...
    sed "s|@INSTALL_DIR@|$linux_install_dir|g" \
        packaging/linux/factorseal-agent.service.in \
        > "$stage/$archive/share/systemd/user/factorseal-agent.service"
    cp packaging/linux/factorseal-agent.service.in "$stage/$archive/share/systemd/user/"
    chmod 0755 \
        "$stage/$archive/bin/factorseal" \
        "$stage/$archive/bin/factorseal-agent-start"
else
    app="$stage/$archive/Factorseal Agent.app/Contents"
    ...
    cp packaging/macos/dev.factorseal.agent.plist "$stage/$archive/Library/LaunchAgents/"
...
if [ "$platform" = macos ]; then
    ...
    cp -R "$stage/$archive/Factorseal Agent.app" "$package_root/Applications/"
    cp packaging/macos/dev.factorseal.agent.plist "$package_root/Library/LaunchAgents/"
    pkgbuild \
        --root "$package_root" \
        --identifier dev.factorseal.agent \
        --version "$version" \
        --install-location / \
