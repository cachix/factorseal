...
//! compared with the agent process SID. Handles are immediately transferred
...
#[cfg(test)]
use super::{AgentClient, WindowsAgentClient};
use super::{
...
pub struct WindowsAgentOptions {
...
pub fn serve_windows_agent(
    service: &Arc<AgentService>,
    options: &WindowsAgentOptions,
) -> AgentResult<()> {
    ...
    let served = accept_until_locked(service, options, &listener, &stopping);
    ...
    while !stopping.load(Ordering::Acquire) {
        ...
        match listener.accept() {
            Ok(mut stream) => {
                ...
                // must not terminate the per-user agent, which is why this
...
impl WindowsLifecycleMonitor {
    fn new(service: Arc<AgentService>, stopping: Arc<AtomicBool>) -> AgentResult<Self> {
        let (ready_sender, ready_receiver) = sync_channel(1);
        let thread = std::thread::Builder::new()
            .name("factorseal-windows-lifecycle".to_owned())
            .spawn(move || lifecycle_message_loop(service, stopping, &ready_sender))
            .map_err(|error| {
                AgentError::Protocol(format!("could not start lifecycle monitor: {error}"))
            })?;
        let thread_id = ready_receiver
            .recv()
            .map_err(|_| {
                AgentError::Protocol("Windows lifecycle monitor stopped during startup".to_owned())
            })?
...
pub fn windows_caller_identity_for_executable(
    ...
    let executable = fs::canonicalize(executable.as_ref()).map_err(|error| {
        AgentError::Protocol(format!("could not canonicalize executable: {error}"))
    })?;
    CallerIdentity::new(
...
fn caller_identity(stream: &BytePipe, cache: &CallerIdentityCache) -> AgentResult<CallerIdentity> {
    ...
    let mut opened = File::open(&executable)
        .map_err(|error| AgentError::Protocol(format!("could not open executable: {error}")))?;
    let metadata = opened
        .metadata()
        .map_err(|error| AgentError::Protocol(format!("could not inspect executable: {error}")))?;
...
fn client_sid(stream: &BytePipe) -> AgentResult<String> {
    ...
    token
        ...
        .map_err(|error| AgentError::Protocol(format!("could not read client SID: {error}")))
}
...
fn current_process_sid() -> AgentResult<String> {
    OwnedToken::from_current_process(TOKEN_QUERY)
        ...
        .and_then(|sid| sid.to_string())
        .map_err(|error| AgentError::Protocol(format!("could not read process SID: {error}")))
}

fn open_current_thread_token() -> AgentResult<OwnedToken> {
...
fn process_executable(process_id: u32) -> AgentResult<(PathBuf, u64)> {
...
fn same_user_security_descriptor() -> AgentResult<SecurityDescriptor> {
    let sddl = format!(
        "D:P(A;;GA;;;{})(A;;GA;;;SY)(A;;GA;;;BA)",
        current_process_sid()?
    );
    let sddl = U16CString::from_str(sddl)
        .map_err(|error| AgentError::Protocol(format!("invalid pipe DACL: {error}")))?;
    SecurityDescriptor::deserialize(&sddl)
        .map_err(|error| io_error("create same-user pipe DACL", &error))
}

fn validate_options(options: &WindowsAgentOptions) -> AgentResult<()> {
    validate_pipe_name(&options.pipe_name)?;
    if options.poll_interval.is_zero() || options.poll_interval > Duration::from_secs(1) {
        return Err(AgentError::Protocol(
            "Windows agent poll interval must be between zero and one second".to_owned(),
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    use crate::{
        AgentAction, AgentResponseBody, AgentStore, GrantPermission, Seal, SealId,
        UnlockLeasePolicy, WireSecret, WireSecretAddress,
    };
    ...
    fn a_failing_event_loop_still_locks_the_store() {
        ...
        let options = WindowsAgentOptions {
            pipe_name: format!(r"\\.\pipe\factorseal-test-{}", SealId::random().unwrap()),
            poll_interval: Duration::from_millis(5),
            install_signal_handler: false,
            install_lifecycle_monitor: false,
        };
        assert!(serve_windows_agent(&service, &options).is_err());
        assert!(
    ...
    fn production_options_require_lifecycle_monitoring() {
        let options = WindowsAgentOptions::new(r"\\.\pipe\factorseal-device-id");
    ...
    fn native_transport_round_trips_and_locks() {
        ...
        let pipe_name = format!(r"\\.\pipe\factorseal-test-{}", SealId::random().unwrap());
        let options = WindowsAgentOptions {
        ...
        let server = std::thread::spawn(move || serve_windows_agent(&server_service, &options));
        let client = WindowsAgentClient::new(pipe_name);
    ...
    fn wait_until_ready(
        ...
        assert!(
            server.is_finished(),
            "the Windows agent is still serving but unreachable: {last:?}"
        );
        panic!("the Windows agent thread exited during startup: {last:?}");
    }
