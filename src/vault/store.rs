...
fn commit_transcript(
    ...
    append_bytes(&mut bytes, scope.as_str().as_bytes());
    bytes.extend_from_slice(&generation.to_be_bytes());
    bytes.extend_from_slice(&key_epoch.to_be_bytes());
    bytes.extend_from_slice(&snapshot_digest);
    bytes.extend_from_slice(&changes_digest);
    bytes.extend_from_slice(device_key_id.as_bytes());
    bytes
}
...
fn digest_change_envelopes(envelopes: &[SignedChangeEnvelope], serialized: &[Vec<u8>]) -> [u8; 32] {
